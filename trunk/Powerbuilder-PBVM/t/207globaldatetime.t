...
my $id = $session->GetGlobalVarID("gdt_var" );
cmp_ok( $id, 'ne', 0,    'get var id');

SKIP:{
	skip "Get<type>GlobalVar not yet implemented.", 4 
		unless $session->can('GetDateTimeGlobalVar');
	my $isnull = 0;
	my $var = $session->GetDateTimeGlobalVar( $id, $isnull );
	...
	my $pbdatetime = $session->NewDateTime();
	cmp_ok( $session->SetDateTimeGlobalVar($id, $pbdatetime), '==', 0, 'var set');
}