...
my $id = $session->GetGlobalVarID("gul_var" );
cmp_ok( $id, 'ne', 0,    'get var id');

SKIP:{
	skip "Get<type>GlobalVar not yet implemented.", 4 
		unless $session->can('GetUlongGlobalVar');
	my $isnull = 0;
	my $var = $session->GetUlongGlobalVar( $id, $isnull );
	cmp_ok( $var, 'eq', 100, 'var get');
	cmp_ok( $isnull, '==', 0, 'var get (isnull)');
	$session->SetGlobalVarToNull($id);
	cmp_ok( $session->IsGlobalVarNull($id), '==', 1, 'var set null');
	cmp_ok( $session->SetUlongGlobalVar($id, 35), '==', 0, 'var set');
}