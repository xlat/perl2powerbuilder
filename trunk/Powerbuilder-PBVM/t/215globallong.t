...
my $id = $session->GetGlobalVarID("gl_var" );
cmp_ok( $id, 'ne', 0,    'get var id');

SKIP:{
	skip "Get<type>GlobalVar not yet implemented.", 4 
		unless $session->can('GetLongGlobalVar');
	my $isnull = 0;
	my $var = $session->GetLongGlobalVar( $id, $isnull );
	...
	cmp_ok( $session->SetLongGlobalVar($id, 35), '==', 0, 'var set');
}