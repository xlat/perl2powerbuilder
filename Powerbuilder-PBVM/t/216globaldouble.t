...
my $id = $session->GetGlobalVarID("gdo_var" );
cmp_ok( $id, 'ne', 0,    'get var id');

SKIP:{
	skip "Get<type>GlobalVar not yet implemented.", 4 
		unless $session->can('GetDoubleGlobalVar');
	my $isnull = 0;
	my $var = $session->GetDoubleGlobalVar( $id, $isnull );
	cmp_ok( $var, 'eq', -45.5, 'var get');
	cmp_ok( $isnull, '==', 0, 'var get (isnull)');
	$session->SetGlobalVarToNull($id);
	cmp_ok( $session->IsGlobalVarNull($id), '==', 1, 'var set null');
	cmp_ok( $session->SetDoubleGlobalVar($id, 3.5), '==', 0, 'var set');
}