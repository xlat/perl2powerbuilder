...
my $id = $session->GetGlobalVarID("gt_var" );
cmp_ok( $id, 'ne', 0,    'get var id');

SKIP:{
	skip "Get<type>GlobalVar not yet implemented.", 4 
		unless $session->can('GetTimeGlobalVar');
	my $isnull = 0;
	my $var = $session->GetTimeGlobalVar( $id, $isnull );
	...
	my $pbtime = $session->NewTime();
	cmp_ok( $session->SetTimeGlobalVar($id, $pbtime), '==', 0, 'var set');
}