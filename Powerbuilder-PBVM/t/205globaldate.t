...
my $id = $session->GetGlobalVarID("gd_var" );
cmp_ok( $id, 'ne', 0,    'get var id');

SKIP:{
	skip "Get<type>GlobalVar not yet implemented.", 4 
		unless $session->can('GetDateGlobalVar');
	my $isnull = 0;
	my $var = $session->GetDateGlobalVar( $id, $isnull );
	...
	my $pbdate = $session->NewDate();
	cmp_ok( $session->SetDateGlobalVar($id, $pbdate), '==', 0, 'var set');
}