...
my $id = $session->GetGlobalVarID("gui_var" );
cmp_ok( $id, 'ne', 0,    'get var id');

SKIP:{
	skip "Get<type>GlobalVar not yet implemented.", 4 
		unless $session->can('GetUintGlobalVar');
	my $isnull = 0;
	my $var = $session->GetUintGlobalVar( $id, $isnull );
	cmp_ok( $var, 'eq', 2, 'var get');
	cmp_ok( $isnull, '==', 0, 'var get (isnull)');
	$session->SetGlobalVarToNull($id);
	cmp_ok( $session->IsGlobalVarNull($id), '==', 1, 'var set null');
	cmp_ok( $session->SetUintGlobalVar($id, 35), '==', 0, 'var set');
}